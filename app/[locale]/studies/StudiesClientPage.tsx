...
import { studies as studyDetails } from "@/data/studies";
...
type StudiesClientPageProps = {
  initialPage: number;
};

type StudyFilterCategory = "Todos" | StudyCategory;

type StudyListItem = StudyDetail & {
  href: string;
};

const PAGE_SIZE = 8;
const FEATURED_STUDY_SLUG = "sua-operacao-pode-estar-funcionando";

const categoryFilters: {
  label: StudyFilterCategory;
  icon: ElementType;
}[] = [
...
const trails = [
  {
    title: "Fundamentos",
    description:
      "Conceitos centrais sobre saúde da loja digital, sinais vitais, janelas de atenção e perda de consistência.",
    icon: Activity,
    href: "#lista",
    items: [
      "O que é saúde da loja digital?",
...
function getLocaleBasePath(pathname: string) {
  const normalizedPathname = pathname.replace(/\/$/, "");

  if (normalizedPathname.endsWith("/studies")) {
    return normalizedPathname.replace(/\/studies$/, "");
  }

  return "";
}

function withStudyHref(
  study: StudyDetail,
  studiesBasePath: string
): StudyListItem {
  return {
    ...study,
    href: `${studiesBasePath}/${study.slug}`,
  };
}

export function StudiesClientPage({ initialPage }: StudiesClientPageProps) {
  ...
  const studiesBasePath = `${localeBasePath}/studies`;
  const diagnosticHref = `${localeBasePath}/diagnostic`;
  const contactHref = `${localeBasePath}/contact`;

  const [selectedCategory, setSelectedCategory] =
    useState<StudyFilterCategory>("Todos");
  const [searchQuery, setSearchQuery] = useState("");
  const [currentPage, setCurrentPage] = useState(initialPage);

  const allStudies = useMemo(
    () =>
      studyDetails.map((study) => withStudyHref(study, studiesBasePath)),
    [studiesBasePath]
  );

  const featuredStudy = useMemo(() => {
    return (
      allStudies.find((study) => study.slug === FEATURED_STUDY_SLUG) ??
      allStudies[0]
    );
  }, [allStudies]);
  ...
  }, [allStudies, selectedCategory, searchQuery]);
  ...
  }, [filteredStudies, safeCurrentPage]);

  function replacePageParam(nextPage: number) {
    const nextUrl =
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-100 bg-violet-50 px-3 py-1 text-sm font-semibold text-violet-900">
                ...
                Como estimamos a saúde da loja digital
              </p>
    ...
    <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white">
        ...
        <table className="w-full min-w-[860px] border-collapse text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {studies.map((study) => {
              ...
              const isAvailable = status !== "in_production";
              ...
              return (
                ...
                >
                  ...
                  <td className="px-5 py-5 align-top">
                    <StudyTableStatusBadge status={status} />
                  </td>
                </tr>
              );
            })}
      ...
      <div className="flex flex-col gap-4 border-t border-slate-100 bg-slate-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm font-semibold text-slate-500">
          Mostrando <span className="text-slate-800">{firstItem}</span> –{" "}
          <span className="text-slate-800">{lastItem}</span> de{" "}
          <span className="text-slate-800">{totalCount}</span> estudos
        </p>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex h-10 cursor-pointer items-center justify-center rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-600 transition hover:border-violet-200 hover:text-violet-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Anterior
          </button>
            ...
            {Array.from({ length: pageCount }).map((_, index) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-sm font-semibold transition",
                    isCurrent
...
function FeaturedStudyCard({ study }: { study: StudyListItem }) {
  const Icon = studyIcons[study.icon];
  const status = study.status ?? "published";
  const isAvailable = status !== "in_production";
  ...
  return (
    <article className="grid grid-cols-1 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm lg:grid-cols-[0.95fr_1.05fr]">
      <div
        className={`relative min-h-[280px] bg-gradient-to-br ${study.visualClass} p-8 text-white`}
      >
...
function TrailCard({
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-5 text-xl font-semibold text-violet-800">
        {trail.title}
      </h3>
